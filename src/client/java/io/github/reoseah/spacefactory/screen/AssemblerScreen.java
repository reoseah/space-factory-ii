...
public class AssemblerScreen extends GhostSlotMachineScreen<AssemblerRecipe, AssemblerScreenHandler> {
    ...
    @Override
    protected ItemStack getOutput(AssemblerRecipe recipe) {
    ...
    protected void drawBackground(DrawContext context, float delta, int mouseX, int mouseY) {
        ...
        int duration = this.handler.properties.get(GhostSlotMachineScreenHandler.Properties.RECIPE_ENERGY);
        if (duration == 0) {
            duration = 10000;
        }
        int progressToDisplay = this.handler.properties.get(GhostSlotMachineScreenHandler.Properties.RECIPE_PROGRESS) * 24 / duration;
    ...
    protected void drawMouseoverTooltip(DrawContext context, int mouseX, int mouseY) {
        if (this.isPointWithinBounds(103, 26, 24, 16, mouseX, mouseY)) {
            int recipeEnergy = this.handler.properties.get(GhostSlotMachineScreenHandler.Properties.RECIPE_ENERGY);
            int progress = this.handler.properties.get(GhostSlotMachineScreenHandler.Properties.RECIPE_PROGRESS);
