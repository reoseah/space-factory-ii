...
public class AssemblerScreen extends HandledScreen<AssemblerScreenHandler> {
    ...
    protected void drawMouseoverTooltip(DrawContext context, int mouseX, int mouseY) {
        ...
        if (this.isPointWithinBounds(103, 26, 24, 16, mouseX, mouseY)) {
            int recipeEnergy = this.handler.properties.get(AssemblerScreenHandler.Properties.RECIPE_ENERGY);
    ...
    protected void drawRecipeBackground(DrawContext context, int mouseX, int mouseY) {
        for (int i = this.scrollOffset; i < this.scrollOffset + 12 && i < this.handler.getAvailableRecipes().size(); i++) {
            ...
            context.drawTexture(TEXTURE, x, y, 176, v, 18, 18);
        }
    }

    protected void drawRecipeIcons(DrawContext context) {
        List<AssemblerRecipe> recipes = this.handler.getAvailableRecipes();
        ...
        for (int i = this.scrollOffset; i < this.scrollOffset + 12 && i < recipes.size(); i++) {
            ...
            context.drawItem(recipes.get(i).output, x, y, i);
        }
    }
    ...
    protected void drawGhostSlots(DrawContext context, float delta) {
        ...
        AssemblerRecipe recipe = this.handler.getAvailableRecipes().get(this.handler.getSelectedRecipeIdx());
...
//                        this.setZOffset(this.getZOffset() - 100);
    ...
    protected boolean drawGhostSlotTooltip(DrawContext context, int mouseX, int mouseY) {
        AssemblerRecipe recipe = this.handler.getAvailableRecipes() //
